...
from crawler.rss_crawler import DEFAULT_DATETIME, httpRequest, parse_datetime
...
class myThread(threading.Thread):
    def __init__(self, article: Article, db_sess, use_selenium):
        threading.Thread.__init__(self)
        self.session = db_sess
        self.article = article
        host = urlparse(article.link_url)[1]
        self.media: Media = (
    ...
    def run(self):
        try:
            ...
            self.exit_code = 1
            logger.error(f"异常url: {self.article.link_url}, {e}")
...
def thread_pool_worker(article):
    engine = get_db_engine()
    db_sess = sessionmaker(bind=engine)()
    try:
        thread = myThread(article=article, db_sess=db_sess, use_selenium=False)
        thread.start()
    finally:
        db_sess.close()
        engine.dispose()
    ...
    with ThreadPoolExecutor(max_workers=THREAD_MAX) as pool:
        try:
            while True:
                ...
                articles = db_sess.query(Article).filter(Article.s3_prefix == "").all()
                for article in articles:
                    pool.submit(thread_pool_worker, article)
...
if __name__ == "__main__":
    ...
    #     for article in articles:
    #         thread = myThread(
    #             article=article, db_sess=db_sess, use_selenium=use_selenium
    #         )
    #         thread.run()
    # finally:
    #     db_sess.close()
